...
`bookdown` is a powerful tool for creating not only books, but also other kinds of documents with cross-referencing.
`papaja` is specifically designed for APA formatting, which is used in my field.

Although many researchers rely on Word-based work flows, I found these existing resources for combining R Markdown and Microsoft Word to be limited.
...
As a result, these packages do not fully support the flexible and customizable Word outputs needed for writing scientific manuscripts.
